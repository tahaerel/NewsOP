    ...
    <header className="mb-4">
      <nav className="p-4 flex items-center">
        <div className="flex items-center mr-4">
          ...
          <span className="ml-3 font-semibold text-xl">NewsOP</span>
        </div>
...
function PopupActions() {
  ...
  const load = useCallback(async () => {
    ...
    if (!tab) throw new Error("Geçerli sekme algılanamadı");
  ...
  const startRecord = useCallback(async () => {
    ...
    try {
      if (!activeTab) throw new Error("Geçerli sekme algılanamadı");
        ...
        {tabInfo && (
          <>
            ...
            <p className="px-4 pb-1 prose prose-slate leading-snug opacity-75">
              <span>Bu tarayıcıdan 🔉 Herhangi bir sesi metne dönüştürün  📑</span>
              <br />
              <span className="">Başlamak için tıklayın! ⚡️</span>
            </p>
...
async function record(tab: chrome.tabs.Tab, recordType: RecordType) {
  if (tab.url?.includes(chrome.runtime.id))
    throw new Error("NewsOP Sekmesinde başlatılamıyor");
