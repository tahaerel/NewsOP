...
export default function Prompts({ setIsOpen, docPrompt, setDocPrompt, setting, setSetting, onSaveDocPrompt }) {
    ...
    return <div className=''>
        ...
        <div className="navigation flex items-center overflow-auto left-1 relative container mb-4">
            <div className="whitespace-nowrap py-2 group relative">
                <div className={tabClass('global', tab)} onClick={(e) => setTab('global')}>
                    ...
                    <div className="navigation-link">Genel</div>
                </div>
            </div>
            <div className="whitespace-nowrap py-2 group relative">
                <div className={tabClass('doc', tab)} onClick={(e) => setTab('doc')}>
                    ...
                    <div className="navigation-link">Belge</div>
                </div>
            </div>
            <div className="whitespace-nowrap py-2 group relative">
                <div className={tabClass('action', tab)} onClick={(e) => setTab('action')}>
                    ...
                    <div className="navigation-link">Aksiyon</div>
                </div>
            </div>
        </div>
        ...
        <div className="flex flex-col">
            <textarea id="docPrompt" name="docPrompt" className='textarea' placeholder='Döküman promptu girin' rows="8" value={docPrompt} onChange={(e) => setDocPrompt(e.target.value)}></textarea>
        </div>
        }
        ...
        <li className='rounded-md border border-transparent bg-gray-100 px-4 py-2 text-sm font-medium text-graylue-900 hover:bg-blue-200 hover:text-blue-900 cursor-pointer mt-6' onClick={onCreateAction}>Aksiyon Yarat</li>
        </ul>
        <div className='w-3/4'>
            <label for="name" className="text-left text-gray-700">İsim</label>
            <input id="name" name='name' className='input' value={setting.actionPrompts[actionIndex].name} onChange={handleAPChange}/>
            <label for="prompt" className="text-left text-gray-700 mt-2">Aksiyon Promptu</label>
            <textarea id="prompt" name='prompt' className='textarea' placeholder='Aksiyon Promptu Girin' rows="8" value={setting.actionPrompts[actionIndex].prompt} onChange={handleAPChange}/>
        </div>
        </div>
        }
