  ...
  return (
    ...
    <PanelGroup direction="horizontal">
      ...
      <div className="editor-container  min-h-full">
        ...
        <input className="flex px-4 w-full text-4xl outline-none" contentEditable="true" placeholder="Başlık" value={title} onChange={e => {setTitle(e.target.value); onTitleChange(e.target.value);}}></input>
...
export default function App(){
  ...
  useEffect(() => {
      if(currentDoc === null){
        ...
        if(docId === undefined || docId === null){
          ...
          onSelectDoc(docId)
          console.log("Yeni haber yükleniyor", docId, currentDoc)
  ...
  const onChatUpdate = (id:string, role:Role, response:string) => {
    ...
    console.log("Geçmiş kaydediliyor..", _history)
  };
  ...
  const onChange = (editorState: EditorState) => {
      if(currentDoc){
        ...
        console.log("Değişiklikler kaydediliyor...", history)
      }
  }
  ...
  // }
  const onCreateDoc = () => {
    const doc: Doc = {id: nanoid(), title: "Başlık", prompt: "", data: defaultData, history: [], createdAt: +new Date, updatedAt: +new Date}
  ...
  const onExport = (e) => {
    downloadObjectAsJson(localStorage, "newsop-"+new Date().toISOString().replaceAll(':', '.'))
  }
  ...
  return (
    <div style={{ fontFamily: 'Roboto, sans-serif', color: '#333', backgroundColor: '#F4F7FA' }}>
      <Modal title="Promptlar" isOpen={isPromptsOpen} setIsOpen={setIsPromptsOpen} style={{ borderRadius: '8px', boxShadow: '0 4px 8px rgba(0, 0, 0, 0.1)' }}>
      ...
      <div className="sticky top-0 z-10 flex items-center border-b border-gray-300 pl-1 pt-1 bg-white text-gray-800 lg:pl-3">
        <button type="button" onClick={onMobileRightClick} className="p-2 inline-flex items-center justify-center rounded-md text-gray-800 hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-blue-500 cursor-pointer">
          <span className="sr-only">Open sidebar</span>
          {(!isNavOpen && !isChatOpen) && <FiMenu size="1.5rem" />}
        </button>
        <h1 className="flex-1 text-center text-lg font-bold">NewsOP Editör</h1>
        <button type="button" className="px-3 hover:bg-gray-200 cursor-pointer" onClick={onMobileLeftClick}>
          {(isNavOpen || isChatOpen) && <FiX size="1.5rem" />}
          {(!isNavOpen && !isChatOpen) && <FiMessageSquare size="1.5rem" />}
        </button>
      </div>
  
      <div className="flex">
        <Modal title="Ayarlar" isOpen={isSettingsOpen} setIsOpen={setIsSettingsOpen} style={{ borderRadius: '8px', boxShadow: '0 4px 8px rgba(0, 0, 0, 0.1)' }}>
          <Settings setIsOpen={setIsSettingsOpen} setting={setting} setSetting={setSetting}/>
        </Modal>
  
        <div className={`${isNavOpen ? "w-full z-10 p-4 border-r border-gray-300 bg-white" : "hidden"} lg:flex lg:flex-col lg:w-1/5 lg:p-4`}>
          <Link href="/"><h1 className="text-4xl font-extrabold text-blue-600 cursor-pointer">NewsOP</h1></Link>
          <div className="text-gray-800 pb-4">New Generation News Editor</div>
          <a onClick={onCreateDoc} className="flex p-3 items-center gap-3 transition-colors duration-200 rounded-md border border-blue-600 hover:bg-blue-500 hover:text-white text-blue-600 cursor-pointer">
            Yeni Haber
          </a>
          ...
          <ol>
            {docs.map((item, i) => (
              <li key={item.id}>
                <a onClick={() => onSelectDoc(item.id)} className="flex p-3 gap-3 items-center group rounded-md text-blue-800 hover:bg-blue-400 hover:text-white cursor-pointer">
                  {item.title}
                </a>
              </li>
            ))}
          ...
          <div className="flex-col flex-1 border-t border-gray-300 pt-2 mt-2">
            <a onClick={onExport} className="flex p-3 gap-3 items-center group rounded-md text-blue-800 hover:bg-blue-400 hover:text-white cursor-pointer">
              <FiDownload/> Kaydet
            </a>
            <input id="import-file" className="sr-only" type="file" accept=".json" onChange={onImportChange}/>
            <a onClick={() => {
              const importFile = document.querySelector('#import-file');
              if (importFile) importFile.click();
            }} className="flex p-3 gap-3 items-center group rounded-md text-blue-800 hover:bg-blue-400 hover:text-white cursor-pointer">
              <FiUpload/> İçe Aktar
            </a>
            <a onClick={(e) => setIsSettingsOpen(true)} className="flex p-3 gap-3 items-center group rounded-md text-blue-800 hover:bg-blue-400 hover:text-white cursor-pointer">
              <FiSettings/> Ayarlar
            </a>
          </div>
        </div>
